class ApiLoopInsController < ApplicationController
  ...
  def create
    ...
    if target_post
      if target_post.add_voter(current_user)
        if target_post.save
          ...
          render json: build_ajax_response(:ok, nil, "You looped into #{target_user.first_name}'s night", nil, { }), status: 201
        else
          render json: build_ajax_response(:error, nil, "Unable to loop in. Please contact support@thewhoot.com."), status: 401
        end
      else
        render json: build_ajax_response(:error, nil, "You cannot loop in to your own night"), status: 401
      end
    else
      render json: build_ajax_response(:error, nil, 'Target post not found!'), status: 404
    end
  end
