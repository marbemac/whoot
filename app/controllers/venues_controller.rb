class VenuesController < ApplicationController
  ...
  def attending
    @venue = Venue.find(params[:id])
    if @venue
      posts = Post.where('venue._id' => @venue.id, :user_id.in => current_user.following_users, :current => true, :created_at.gte => Chronic.parse('today at 5:00am', :now => (Time.now - (60*60*5))))
