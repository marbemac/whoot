class VotesController < ApplicationController
  ...
  def create
    ...
    if target_post
      ...
      if target_post.save
        @pubnub.publish({'channel' => target_post.user_id.to_s, 'message' => {:event => 'voted', :user_id => target_post.user_id.to_s, :votes => target_post.votes, :post_id => target_post.id.to_s}})
        @pubnub.publish({'channel' => target_post.user_id.to_s+'_private', 'message' => {:event => 'notification', :content => "#{current_user.fullname} +1'd your post."}})
      end
